...
class ZXPrism:
    ...
    @staticmethod
    def patch_triangle_bdry(
        d: int, left_corner: Position3DHex, triangle_type: str
    ) -> dict[str, list[Position3DHex]]:
        ...
        return {
            ...
            "b": dirb,
            "c": dirc,
        }  # dira turned around for consistency in reduce_weight_six_to_four
    ...
    def patch_adjacent_bulk_stabilizers(init_point: Position3DHex) -> list[list[Position3DHex]]:
        ...
        if init_point.x % 2 == 0:
            # note that this choice is not unique as
            # some directions yield same results for this case
            plaq1 = [init_point]
    ...
    @staticmethod
    def check_within_bdrys(
        pos: Position3DHex, bdry_dct: dict[str, list[Position3DHex]], d: int
    ) -> bool:
    ...
    @staticmethod
    def remove_duplicate_stabilizers(
    ...
    @staticmethod
    def remove_low_overlap_stabilizers(
        stabilizers: list[list[Position3DHex]], bdry_dct: dict[str, list[Position3DHex]], d: int
    ) -> list[list[Position3DHex]]:
    ...
    @staticmethod
    def reduce_weight_six_to_four(
        stabilizers: list[list[Position3DHex]],
        reduce_bdry: list[str],
        nodes_triangle_bdry: dict[str, list[Position3DHex]],
        d: int,
    ) -> list[list[Position3DHex]]:
        ...
        for el in reduce_bdry:
            for node in nodes_triangle_bdry[el][
                :-1
            ]:  # depends on the order of the boundary node lists
                for i, stab in enumerate(stabilizers):
                    bool_lst = [
                        ZXPrism.check_within_bdrys(node, nodes_triangle_bdry, d) for node in stab
                    ]
                    num_inside = np.sum([1 for el in bool_lst if el])
    ...
    def patch_stabilizers(
        ...
        while len(appear_once_within) != 0:
            ...
            appear_once_within = [
                p for p in appear_once if ZXPrism.check_within_bdrys(p, nodes_triangle_bdry, d)
            ]
        ...
        stabilizers = ZXPrism.reduce_weight_six_to_four(
            stabilizers, reduce_bdry, nodes_triangle_bdry, d
        )
        ...
        if len(stabilizers) != (3 * (d**2 - 1)) / 8:
            raise TQECError(
                "Internal issue with patch stabilizer construction."
    ...
    def order_stabilizer(stabilizer: list[Position3DHex]):
        ...
        start = next(
            (
                pos
                for pos in stabilizer
                if sum(1 for other in stabilizer if pos.is_neighbour(other)) == 1
            ),
            stabilizer[0],
        )
    ...
    @staticmethod
    def find_pairs_with_two_overlaps(
        stabilizers: list[list[Position3DHex]],
    ) -> list[list[Position3DHex]]:
    ...
    def star_operator_patch(triangle_type: str, nodes_triangle_bdry: dict):
        ...
        if d == 3:
            star_op = []
            for dir in ["a", "b", "c"]:  # element with index 1 is middle
                star_op.append(nodes_triangle_bdry[dir][1])  # noqa: PERF401
            return star_op
        elif d == 5:
            star_op = []
            ...
            for dir in ["a", "b", "c"]:
                ...
                for neigh in middle_node.neighbors_spatial():
                    if neigh not in bdry_nodes and ZXPrism.check_within_bdrys(
                        neigh, nodes_triangle_bdry, d
                    ):
                ...
                for idx_c in middle_out(len(nodes_triangle_bdry["c"])):
                    ...
                    if intersection:
                        ...
                        check_ok_middle = (
                            ...
                            and len(arms[1]) % 2 == 0
                            and len(arms[2]) % 2 == 0
                        )  # even if middle point not included
                        thres = 0
                        check_ok_len = (
                            len(arms[0]) > thres and len(arms[1]) > thres and len(arms[2]) > thres
                        )
    ...
    @staticmethod
    def reflect_node(
        node: Position3DHex, direction: str, triangle_type: str, nodes_triangle_bdry: dict
    ):
        ...
        def get_reflection_shift(
            el: Position3DHex, direction: str, triangle_type: str
        ) -> Position3DHex:
    ...
    @staticmethod
    def reflect_star_operator(
        star_op: list[Position3DHex], direction: str, triangle_type: str, nodes_triangle_bdry: dict
    ):
        """Reflect the star operator along the given direction.

        Note that triangle_type and nodes_triangle_bdry must fit to the current star op,
